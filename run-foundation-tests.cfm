<cfscript>
	// Test runner for Task Group 1: Foundation Components
	// Run: http://localhost:8080/run-foundation-tests.cfm
	...
	// Execute pipeline
	writeOutput("<pre>");
	writeOutput("Running Foundation Component Tests" & chr(10));
	writeOutput("==================================" & chr(10) & chr(10));

	// Discover all tests
	allTests = discovery.discover();

	// Filter to only foundation tests
	foundationTests = [];
	targetTests = ["DatabaseConnectionTest", "SeederTest"];
	...
	for (testDescriptor in allTests) {
		...
		for (targetTest in targetTests) {
			if (findNoCase(targetTest, componentName) > 0) {
				arrayAppend(foundationTests, testDescriptor);
				break;
			}
		}
	}

	writeOutput("Discovered " & arrayLen(foundationTests) & " foundation test file(s)" & chr(10));
	writeOutput(chr(10));
	...
	for (testDescriptor in foundationTests) {
